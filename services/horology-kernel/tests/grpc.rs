...
#[tokio::test]
async fn grpc_report_execution_updates_timer() {
    let kernel = HorologyKernel::new(SchedulerConfig::default());
    let service = HorologyKernelService::new(kernel.clone());
    ...
    let scheduled = client
        .schedule_timer(tonic::Request::new(TimerScheduleRequest {
            ...
            requested_by: "agent-test".into(),
            name: "report".into(),
            schedule_time: Some(timer_schedule_request::ScheduleTime::DurationMs(10)),
            metadata_json: String::new(),
            labels: HashMap::new(),
            action_bundle_json: String::new(),
            agent_binding_json: String::new(),
    ...
    let response = client
        .report_timer_execution(tonic::Request::new(TimerExecutionReportRequest {
            tenant_id: "tenant-test".into(),
            timer_id: scheduled.id.clone(),
            final_status: TimerStatus::Failed as i32,
            result: None,
            error: Some(horology_kernel::pb::ExecutionError {
                message: "integration failure".into(),
                code: "HTTP_500".into(),
                metadata_json: String::new(),
            }),
        }))
    ...
    assert_eq!(response.status, TimerStatus::Failed as i32);
    assert_eq!(
        response
            .execution_error
            .and_then(|error| if error.message.is_empty() { None } else { Some(error.message) })
            .unwrap_or_default(),
        "integration failure"
    );
