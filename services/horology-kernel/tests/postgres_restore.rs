...
async fn restores_scheduled_timer_after_restart() {
    ...
    store
        .upsert(&timer)
        .await
        .expect("persist scheduled timer");
    ...
    let command_rows: Vec<(String, Uuid, String)> = sqlx::query(
        "SELECT command, timer_id, tenant_id FROM timer_command_log ORDER BY id",
    )
    .fetch_all(&pool)
    .await
    .expect("fetch command log entries")
    .into_iter()
    .map(|row| (row.get("command"), row.get("timer_id"), row.get("tenant_id")))
    .collect();
    ...
    assert_eq!(
        command_rows,
        vec![
            ("fire".to_string(), expected_timer_id, expected_tenant_id.clone()),
            (
                "settle".to_string(),
                expected_timer_id,
