...
use horology_kernel::{
    EventEnvelope, EventSigner, HorologyKernel, KernelRuntimeOptions, SchedulerConfig, TimerEvent,
    TimerInstance, TimerStatus,
};
use sqlx::Row;
...
async fn restores_scheduled_timer_after_restart() {
    ...
    let mut options = KernelRuntimeOptions::new(shared_store.clone());
    options.command_log = Some(shared_command_log.clone());
    ...
    timeout(Duration::from_secs(3), async {
        loop {
            match events.recv().await {
                Ok(EventEnvelope {
                    event: TimerEvent::Settled(settled),
                    ..
                }) if settled.id == expected_timer_id => break,
                Ok(_) => continue,
                Err(error) => panic!("event channel closed unexpectedly: {error:?}"),
            }
        }
