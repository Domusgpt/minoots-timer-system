...
async fn restores_scheduled_timer_after_restart() {
    ...
    let kernel = HorologyKernel::with_runtime(
        ...
        KernelRuntimeOptions::new(shared_store.clone()),
    )
    .await
    ...
    assert_eq!(fired.id, timer.id);
    assert_eq!(fired.tenant_id, timer.tenant_id);

    timeout(Duration::from_secs(3), async {
        loop {
            match events.recv().await {
                Ok(TimerEvent::Settled(settled)) if settled.id == timer.id => break,
                Ok(_) => continue,
                Err(error) => panic!("event channel closed unexpectedly: {error:?}"),
            }
        }
