...
impl HorologyKernel {
    ...
    pub async fn schedule(&self, spec: TimerSpec) -> Result<TimerInstance, KernelError> {
        ...
        self.state.store.upsert(&timer).await.map_err(KernelError::from)?;
    ...
    pub async fn cancel(
        ...
        self.state.store.upsert(&snapshot).await.map_err(KernelError::from)?;
...
mod tests {
    ...
    async fn schedule_and_fire_emits_events() {
        ...
        assert_eq!(events.recv().await.unwrap(), TimerEvent::Scheduled(timer.clone()));
