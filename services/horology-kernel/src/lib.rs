use std::{collections::HashMap, sync::Arc, time::Duration};

use async_trait::async_trait;
use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};
use thiserror::Error;
use tokio::{sync::broadcast, sync::RwLock};
use tracing::{error, Instrument};
use uuid::Uuid;

pub mod pb {
    tonic::include_proto!("minoots.timer.v1");
}

pub mod grpc;

#[derive(Debug, Error)]
pub enum StorageError {
    #[error("storage operation failed: {0}")]
    Operation(String),
}

#[async_trait]
pub trait TimerStorage: Send + Sync {
    async fn save_timer(&self, timer: &TimerInstance) -> Result<(), StorageError>;
    async fn load_timers(&self) -> Result<HashMap<Uuid, TimerInstance>, StorageError>;
}

pub type SharedTimerStorage = Arc<dyn TimerStorage + Send + Sync + 'static>;

#[derive(Default)]
pub struct InMemoryTimerStorage {
    timers: RwLock<HashMap<Uuid, TimerInstance>>,
}

#[async_trait]
impl TimerStorage for InMemoryTimerStorage {
    async fn save_timer(&self, timer: &TimerInstance) -> Result<(), StorageError> {
        let mut store = self.timers.write().await;
        store.insert(timer.id, timer.clone());
        Ok(())
    }

    async fn load_timers(&self) -> Result<HashMap<Uuid, TimerInstance>, StorageError> {
        let store = self.timers.read().await;
        Ok(store.clone())
    }
}

#[derive(Clone, Debug)]
pub struct SchedulerConfig {
    pub max_duration_ms: Option<u64>,
}

impl Default for SchedulerConfig {
    fn default() -> Self {
        Self {
            max_duration_ms: Some(1000 * 60 * 60 * 24 * 30), // 30 days
        }
    }
}

#[derive(Debug, Error)]
pub enum KernelError {
    #[error("duration must be greater than zero")]
    InvalidDuration,
    #[error("fire_at must be in the future")]
    InvalidFireTime,
    #[error("storage error: {0}")]
    Storage(#[from] StorageError),
    #[error("timer not found")]
    NotFound,
    #[error("timer belongs to a different tenant")]
    TenantMismatch,
    #[error("invalid execution status: {0}")]
    InvalidExecutionStatus(String),
}

#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum TimerStatus {
    Scheduled,
    Armed,
    Fired,
    Cancelled,
    Failed,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct TimerSpec {
    pub tenant_id: String,
    pub requested_by: String,
    pub name: Option<String>,
    pub duration_ms: u64,
    pub fire_at: Option<DateTime<Utc>>,
    pub metadata: Option<serde_json::Value>,
    pub labels: HashMap<String, String>,
    pub action_bundle: Option<serde_json::Value>,
    pub agent_binding: Option<serde_json::Value>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct TimerInstance {
    pub id: Uuid,
    pub tenant_id: String,
    pub requested_by: String,
    pub name: String,
    pub duration_ms: u64,
    pub created_at: DateTime<Utc>,
    pub fire_at: DateTime<Utc>,
    pub status: TimerStatus,
    pub metadata: Option<serde_json::Value>,
    pub labels: HashMap<String, String>,
    pub action_bundle: Option<serde_json::Value>,
    pub agent_binding: Option<serde_json::Value>,
    pub fired_at: Option<DateTime<Utc>>,
    pub cancelled_at: Option<DateTime<Utc>>,
    pub cancel_reason: Option<String>,
    pub cancelled_by: Option<String>,
    pub execution_result: Option<ExecutionResult>,
    pub execution_error: Option<ExecutionError>,
}

impl TimerInstance {
    fn is_terminal(&self) -> bool {
        matches!(
            self.status,
            TimerStatus::Fired | TimerStatus::Cancelled | TimerStatus::Failed
        )
    }
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ExecutionResult {
    pub actions: Vec<ActionResult>,
    pub completed_at_iso: Option<String>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ActionResult {
    pub action_id: String,
    pub success: bool,
    pub output: Option<String>,
    pub metadata: Option<serde_json::Value>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct ExecutionError {
    pub message: String,
    pub code: Option<String>,
    pub metadata: Option<serde_json::Value>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(tag = "type", content = "data")]
pub enum TimerEvent {
    Scheduled(TimerInstance),
    Fired { timer: TimerInstance },
    Cancelled {
        timer: TimerInstance,
        reason: Option<String>,
    },
    Failed {
        timer: TimerInstance,
        error: Option<ExecutionError>,
    },
}

#[derive(Clone)]
struct KernelState {
    timers: Arc<RwLock<HashMap<Uuid, TimerInstance>>>,
    event_tx: broadcast::Sender<TimerEvent>,
    config: SchedulerConfig,
    storage: SharedTimerStorage,
}

#[derive(Clone)]
pub struct HorologyKernel {
    state: KernelState,
}

impl HorologyKernel {
    pub fn new(config: SchedulerConfig) -> Self {
        Self::with_storage_internal(config, Arc::new(InMemoryTimerStorage::default()))
    }

    pub async fn with_storage(
        config: SchedulerConfig,
        storage: SharedTimerStorage,
    ) -> Result<Self, StorageError> {
        let kernel = Self::with_storage_internal(config, Arc::clone(&storage));
        let persisted = storage.load_timers().await?;
        kernel.restore_timers(persisted).await;
        Ok(kernel)
    }

    fn with_storage_internal(config: SchedulerConfig, storage: SharedTimerStorage) -> Self {
        let (event_tx, _rx) = broadcast::channel(1024);
        Self {
            state: KernelState {
                timers: Arc::new(RwLock::new(HashMap::new())),
                event_tx,
                config,
                storage,
            },
        }
    }

    async fn restore_timers(&self, timers: HashMap<Uuid, TimerInstance>) {
        let mut guard = self.state.timers.write().await;
        for (id, timer) in timers {
            guard.insert(id, timer.clone());
            if timer.is_terminal() {
                continue;
            }
            drop(guard);
            self.spawn_fire_task(timer.clone());
            guard = self.state.timers.write().await;
        }
    }

    pub fn subscribe(&self) -> broadcast::Receiver<TimerEvent> {
        self.state.event_tx.subscribe()
    }

    pub async fn schedule(&self, spec: TimerSpec) -> Result<TimerInstance, KernelError> {
        let now = Utc::now();
        let delay = if let Some(ts) = spec.fire_at {
            if ts <= now {
                return Err(KernelError::InvalidFireTime);
            }
            (ts - now)
                .to_std()
                .map_err(|_| KernelError::InvalidFireTime)?
        } else {
            if spec.duration_ms == 0 {
                return Err(KernelError::InvalidDuration);
            }
            Duration::from_millis(spec.duration_ms)
        };

        let duration_ms = delay.as_millis() as u64;
        if let Some(max) = self.state.config.max_duration_ms {
            if duration_ms > max {
                return Err(KernelError::InvalidDuration);
            }
        }

        let chrono_delay =
            chrono::Duration::from_std(delay).map_err(|_| KernelError::InvalidFireTime)?;
        let fire_at = spec.fire_at.unwrap_or_else(|| now + chrono_delay);

        let timer = TimerInstance {
            id: Uuid::new_v4(),
            tenant_id: spec.tenant_id.clone(),
            requested_by: spec.requested_by.clone(),
            name: spec
                .name
                .unwrap_or_else(|| format!("timer-{}", now.timestamp_millis())),
            duration_ms,
            created_at: now,
            fire_at,
            status: TimerStatus::Scheduled,
            metadata: spec.metadata.clone(),
            labels: spec.labels.clone(),
            action_bundle: spec.action_bundle.clone(),
            agent_binding: spec.agent_binding.clone(),
            fired_at: None,
            cancelled_at: None,
            cancel_reason: None,
            cancelled_by: None,
            execution_result: None,
            execution_error: None,
        };

        {
            let mut timers = self.state.timers.write().await;
            timers.insert(timer.id, timer.clone());
        }

        self.state.storage.save_timer(&timer).await?;

        let _ = self
            .state
            .event_tx
            .send(TimerEvent::Scheduled(timer.clone()));

        self.spawn_fire_task(timer.clone());

        Ok(timer)
    }

    pub async fn cancel(
        &self,
        tenant_id: &str,
        timer_id: Uuid,
        reason: Option<String>,
        cancelled_by: Option<String>,
    ) -> Option<TimerInstance> {
        let snapshot = {
            let mut timers = self.state.timers.write().await;
            let entry = timers.get_mut(&timer_id)?;
            if entry.tenant_id != tenant_id {
                return None;
            }

            if entry.is_terminal() {
                return Some(entry.clone());
            }

            entry.status = TimerStatus::Cancelled;
            entry.cancelled_at = Some(Utc::now());
            entry.cancel_reason = reason.clone();
            entry.cancelled_by = cancelled_by;
            entry.clone()
        };

        if let Err(err) = self.state.storage.save_timer(&snapshot).await {
            error!(timer_id = %snapshot.id, "failed to persist cancelled timer: {err}");
        }

        let _ = self.state.event_tx.send(TimerEvent::Cancelled {
            timer: snapshot.clone(),
            reason,
        });
        Some(snapshot)
    }

    pub async fn get(&self, tenant_id: &str, timer_id: Uuid) -> Option<TimerInstance> {
        let timers = self.state.timers.read().await;
        timers
            .get(&timer_id)
            .filter(|t| t.tenant_id == tenant_id)
            .cloned()
    }

    pub async fn list(&self, tenant_id: &str) -> Vec<TimerInstance> {
        let timers = self.state.timers.read().await;
        let mut timers: Vec<_> = timers
            .values()
            .filter(|t| t.tenant_id == tenant_id)
            .cloned()
            .collect();
        timers.sort_by_key(|t| t.fire_at);
        timers
    }

    pub async fn report_execution(
        &self,
        tenant_id: &str,
        timer_id: Uuid,
        final_status: TimerStatus,
        result: Option<ExecutionResult>,
        error: Option<ExecutionError>,
    ) -> Result<TimerInstance, KernelError> {
        if !matches!(
            final_status,
            TimerStatus::Fired | TimerStatus::Failed
        ) {
            return Err(KernelError::InvalidExecutionStatus(format!(
                "unsupported final status: {:?}",
                final_status
            )));
        }

        let mut timers = self.state.timers.write().await;
        let entry = timers
            .get_mut(&timer_id)
            .ok_or(KernelError::NotFound)?;

        if entry.tenant_id != tenant_id {
            return Err(KernelError::TenantMismatch);
        }

        if matches!(entry.status, TimerStatus::Cancelled) {
            return Err(KernelError::InvalidExecutionStatus(
                "timer has been cancelled".into(),
            ));
        }

        entry.status = final_status.clone();
        if entry.fired_at.is_none() {
            entry.fired_at = Some(Utc::now());
        }
        entry.execution_result = result.clone();
        entry.execution_error = error.clone();

        let snapshot = entry.clone();
        drop(timers);

        self.state.storage.save_timer(&snapshot).await?;

        let event = match final_status {
            TimerStatus::Failed => TimerEvent::Failed {
                timer: snapshot.clone(),
                error,
            },
            _ => TimerEvent::Fired {
                timer: snapshot.clone(),
            },
        };
        let _ = self.state.event_tx.send(event);

        Ok(snapshot)
    }

    fn spawn_fire_task(&self, timer: TimerInstance) {
        let state = self.state.clone();
        let storage = Arc::clone(&self.state.storage);
        let span = tracing::info_span!("timer_fire_task", timer_id = %timer.id, tenant_id = %timer.tenant_id);
        tokio::spawn(
            async move {
                let now = Utc::now();
                let duration = if timer.fire_at <= now {
                    Duration::from_secs(0)
                } else {
                    (timer.fire_at - now)
                        .to_std()
                        .unwrap_or_else(|_| Duration::from_secs(0))
                };

                tokio::time::sleep(duration).await;

                let mut timers = state.timers.write().await;
                let entry = match timers.get_mut(&timer.id) {
                    Some(entry) => entry,
                    None => return,
                };

                if entry.is_terminal() {
                    return;
                }

                entry.status = TimerStatus::Fired;
                entry.fired_at = Some(Utc::now());
                entry.execution_result = None;
                entry.execution_error = None;
                let snapshot = entry.clone();
                drop(timers);

                if let Err(err) = storage.save_timer(&snapshot).await {
                    error!(timer_id = %snapshot.id, "failed to persist fired timer: {err}");
                }

                let _ = state.event_tx.send(TimerEvent::Fired { timer: snapshot });
            }
            .instrument(span),
        );
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[tokio::test]
    async fn schedule_and_fire_emits_events() {
        tracing_subscriber::fmt::try_init().ok();
        let kernel = HorologyKernel::new(SchedulerConfig::default());
        let mut events = kernel.subscribe();

        let timer = kernel
            .schedule(TimerSpec {
                tenant_id: "tenant-a".into(),
                requested_by: "agent-1".into(),
                name: Some("integration-test".into()),
                duration_ms: 50,
                fire_at: None,
                metadata: None,
                labels: HashMap::new(),
                action_bundle: None,
                agent_binding: None,
            })
            .await
            .expect("schedule timer");

        let scheduled = events.recv().await.expect("scheduled event");
        assert!(matches!(scheduled, TimerEvent::Scheduled(_)));

        let fired = events.recv().await.expect("fired event");
        match fired {
            TimerEvent::Fired { timer: fired_timer } => {
                assert_eq!(fired_timer.id, timer.id);
                assert_eq!(fired_timer.status, TimerStatus::Fired);
            }
            other => panic!("unexpected event: {:?}", other),
        }
    }

    #[tokio::test]
    async fn cancelling_prevents_fire_event() {
        let kernel = HorologyKernel::new(SchedulerConfig::default());
        let mut events = kernel.subscribe();

        let timer = kernel
            .schedule(TimerSpec {
                tenant_id: "tenant-a".into(),
                requested_by: "agent-1".into(),
                name: None,
                duration_ms: 200,
                fire_at: None,
                metadata: None,
                labels: HashMap::new(),
                action_bundle: None,
                agent_binding: None,
            })
            .await
            .unwrap();

        let _ = events.recv().await.expect("scheduled event");

        let cancelled = kernel
            .cancel(
                "tenant-a",
                timer.id,
                Some("manual".into()),
                Some("agent-1".into()),
            )
            .await
            .expect("cancel timer");

        assert_eq!(cancelled.status, TimerStatus::Cancelled);

        let cancel_event = events.recv().await.expect("cancel event");
        match cancel_event {
            TimerEvent::Cancelled {
                timer: cancelled_timer,
                ..
            } => {
                assert_eq!(cancelled_timer.id, timer.id);
            }
            other => panic!("unexpected event: {:?}", other),
        }

        // Ensure no fired event occurs by waiting longer than the duration
        tokio::time::sleep(Duration::from_millis(250)).await;
        while let Ok(event) = events.try_recv() {
            assert!(
                !matches!(event, TimerEvent::Fired { .. }),
                "timer should not emit fired event after cancellation"
            );
        }
    }

    #[tokio::test]
    async fn report_execution_marks_failed_and_emits_event() {
        let kernel = HorologyKernel::new(SchedulerConfig::default());
        let mut events = kernel.subscribe();

        let timer = kernel
            .schedule(TimerSpec {
                tenant_id: "tenant-a".into(),
                requested_by: "agent-1".into(),
                name: Some("failure-test".into()),
                duration_ms: 100,
                fire_at: None,
                metadata: None,
                labels: HashMap::new(),
                action_bundle: None,
                agent_binding: None,
            })
            .await
            .expect("schedule");

        let _ = events.recv().await.expect("scheduled event");

        let error = ExecutionError {
            message: "webhook failed".into(),
            code: Some("HTTP_500".into()),
            metadata: None,
        };

        let updated = kernel
            .report_execution(
                "tenant-a",
                timer.id,
                TimerStatus::Failed,
                None,
                Some(error.clone()),
            )
            .await
            .expect("report execution");

        assert_eq!(updated.status, TimerStatus::Failed);
        assert!(updated.execution_error.is_some());

        match events.recv().await.expect("failed event") {
            TimerEvent::Failed { timer, error: event_error } => {
                assert_eq!(timer.id, updated.id);
                let event_error = event_error.or(timer.execution_error.clone()).expect("error");
                assert_eq!(event_error.message, error.message);
            }
            other => panic!("expected failed event, got {:?}", other),
        }
    }
}
