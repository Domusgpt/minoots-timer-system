            ...
            loop {
                ...
                if held_connection.is_none() {
                    match pool.acquire().await {
                        ...
                        Err(error) => {
                            tracing::error!(?error, "failed to acquire postgres connection for leadership");
                        }
                    }
                } else {
                    // keep lock alive
...
mod tests {
    ...
    async fn elects_single_leader_and_fails_over() {
        ...
        let elector_one = PostgresLeaderElector::new(pool.clone(), 9_001, Duration::from_millis(100));
        let elector_two = PostgresLeaderElector::new(pool.clone(), 9_001, Duration::from_millis(100));
        ...
        assert!(leader_one.is_leader() || leader_two.is_leader(), "no leader elected");
        assert!(
        ...
        assert!(leader_two.is_leader(), "second leader should assume leadership after failover");
    }
}
