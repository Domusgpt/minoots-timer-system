...
use super::{CommandLog, TimerStore};
...
impl CommandLog for PostgresCommandLog {
    async fn append(&self, command: &TimerCommand) -> Result<CommandEntry> {
        ...
        let row = sqlx::query(
            r#"
            INSERT INTO timer_command_log (tenant_id, command_type, command_json)
            VALUES ($1, $2, $3)
            RETURNING sequence, created_at, command_json
            "#,
        )
        .bind(tenant_id)
        .bind(command_type)
        ...
        let command_json: serde_json::Value = row.try_get("command_json")?;
        let command: TimerCommand = serde_json::from_value(command_json)?;
    ...
    async fn load_all(&self) -> Result<Vec<CommandEntry>> {
        let rows = sqlx::query(
            "SELECT sequence, command_json, created_at FROM timer_command_log ORDER BY sequence ASC",
        )
        .fetch_all(&self.pool)
        .await?;
