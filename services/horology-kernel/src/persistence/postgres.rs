...
use super::{command_log::{CommandLog, CommandRecord}, TimerStore};
...
impl CommandLog for PostgresCommandLog {
    async fn append(&self, record: &CommandRecord) -> Result<()> {
        let (tenant_id, timer_id, command) = match record {
            ...
            | CommandRecord::Settle { timer } => (timer.tenant_id.clone(), timer.id, command_name(record)),
            CommandRecord::Fire {
                tenant_id,
                timer_id,
                ..
...
mod tests {
    ...
    async fn appends_command_records() {
        ...
        command_log
            .append(&CommandRecord::Schedule { timer: timer.clone() })
            .await
        ...
        assert_eq!(payload.get("command").and_then(|v| v.as_str()), Some("schedule"));
        assert_eq!(
            payload
                .get("timer")
