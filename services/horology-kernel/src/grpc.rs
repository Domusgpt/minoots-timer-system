...
use crate::pb::{
    self,
    TimerCancelRequest,
    TimerEventStreamRequest,
    TimerExecutionReportRequest,
    TimerGetRequest,
    TimerListRequest,
...
impl HorologyKernelApi for HorologyKernelService {
    ...
    async fn report_timer_execution(
        &self,
        request: Request<TimerExecutionReportRequest>,
    ) -> Result<Response<pb::Timer>, Status> {
        let payload = request.into_inner();
        let timer_id = uuid::Uuid::parse_str(&payload.timer_id)
            .map_err(|_| Status::invalid_argument("timer_id must be a valid UUID"))?;
        let final_status = proto_status_to_kernel(payload.final_status)?;
        let result = parse_execution_result(payload.result).map_err(|err| {
            Status::invalid_argument(format!("invalid execution result payload: {err}"))
        })?;
        let error = parse_execution_error(payload.error).map_err(|err| {
            Status::invalid_argument(format!("invalid execution error payload: {err}"))
        })?;
...
fn to_proto_timer(timer: TimerInstance) -> Result<pb::Timer, Status> {
    Ok(pb::Timer {
        ...
        labels: timer.labels,
        execution_result: timer
            .execution_result
            .map(to_proto_execution_result)
            .transpose()?,
        execution_error: timer
            .execution_error
            .map(to_proto_execution_error)
            .transpose()?,
    })
}
...
fn event_to_proto(event: TimerEvent) -> Result<pb::TimerEvent, Status> {
    match event {
        ...
        TimerEvent::Failed { timer, error } => {
            let proto_timer = to_proto_timer(timer.clone())?;
            let proto_error = error
                .or_else(|| timer.execution_error.clone())
                .map(to_proto_execution_error)
                .transpose()?;
            Ok(pb::TimerEvent {
                event: Some(pb::timer_event::Event::Failed(pb::TimerFailed {
                    timer: Some(proto_timer),
                    error: proto_error,
                })),
            })
        }
...
fn event_belongs_to_tenant(event: &TimerEvent, tenant_id: &str) -> bool {
    match event {
        ...
        TimerEvent::Failed { timer, .. } => timer.tenant_id == tenant_id,
    }
}
...
fn map_kernel_error(error: KernelError) -> Status {
    match error {
        ...
        KernelError::TenantMismatch => Status::permission_denied("timer does not belong to tenant"),
        KernelError::InvalidExecutionStatus(reason) => Status::failed_precondition(reason),
    }
}
...
fn to_proto_execution_result(result: ExecutionResult) -> Result<pb::ExecutionResult, Status> {
    let actions = result
        .actions
        .into_iter()
        .map(|action| {
            Ok(pb::ActionResult {
                ...
                output: action.output.unwrap_or_default(),
                metadata_json: serialize_json(action.metadata)?,
            })
        })
        .collect::<Result<Vec<_>, Status>>()?;

    Ok(pb::ExecutionResult {
        actions,
        completed_at_iso: result.completed_at_iso.unwrap_or_default(),
    })
}

fn to_proto_execution_error(error: ExecutionError) -> Result<pb::ExecutionError, Status> {
    Ok(pb::ExecutionError {
        message: error.message,
        code: error.code.unwrap_or_default(),
        metadata_json: serialize_json(error.metadata)?,
    })
}

fn parse_execution_result(result: Option<pb::ExecutionResult>) -> Result<Option<ExecutionResult>, String> {
    let payload = match result {
        Some(value) => value,
        None => return Ok(None),
    };

    if payload.actions.is_empty() && payload.completed_at_iso.is_empty() {
        return Ok(None);
    }
    ...
    let actions = payload
        ...
        .map(|action| {
            Ok(ActionResult {
                action_id: action.action_id,
                success: action.success,
                output: optional_string(action.output),
                metadata: parse_optional_json_string(action.metadata_json)
                    .map_err(|err| err.message().to_string())?,
            })
        })
        .collect::<Result<Vec<_>, String>>()?;

    let completed_at_iso = if payload.completed_at_iso.trim().is_empty() {
        None
    } else {
        Some(payload.completed_at_iso)
    };
...
fn parse_execution_error(error: Option<pb::ExecutionError>) -> Result<Option<ExecutionError>, String> {
    let payload = match error {
        Some(value) => value,
        None => return Ok(None),
    };

    if payload.message.trim().is_empty() && payload.code.trim().is_empty() && payload.metadata_json.trim().is_empty() {
        return Ok(None);
    }

    let metadata = parse_optional_json_string(payload.metadata_json)
        .map_err(|err| err.message().to_string())?;

    Ok(Some(ExecutionError {
        message: if payload.message.trim().is_empty() {
            "execution failed".to_string()
        } else {
            payload.message
        },
        code: if payload.code.trim().is_empty() {
            None
        } else {
            Some(payload.code)
        },
...
fn proto_status_to_kernel(value: i32) -> Result<TimerStatus, Status> {
    match pb::TimerStatus::try_from(value) {
        Ok(pb::TimerStatus::Fired) => Ok(TimerStatus::Fired),
        Ok(pb::TimerStatus::Failed) => Ok(TimerStatus::Failed),
        Ok(other) => Err(Status::invalid_argument(format!(
            "unsupported execution status: {:?}",
            other
        ))),
        Err(_) => Err(Status::invalid_argument("invalid timer status")),
    }
}
