...
use horology_kernel::persistence::{SharedCommandLog, SharedTimerStore};
...
async fn build_kernel() -> anyhow::Result<HorologyKernel> {
        ...
        .as_str()
    {
        "postgres" => {
            ...
            let command_log = PostgresCommandLog::new(store.pool());
            let shared_log: SharedCommandLog = Arc::new(command_log);
            let kernel = HorologyKernel::with_store(config, shared_store, shared_log).await?;
            tracing::info!(
