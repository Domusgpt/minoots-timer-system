...
async fn main() -> anyhow::Result<()> {
    let telemetry_handle = telemetry::init()?;

    let metrics_addr: SocketAddr = std::env::var("KERNEL_METRICS_ADDR")
        .unwrap_or_else(|_| "0.0.0.0:9464".to_string())
        .parse()
        .with_context(|| "invalid KERNEL_METRICS_ADDR value")?;
    let metrics_task = telemetry_handle.serve_metrics(metrics_addr).await?;
    info!(%metrics_addr, "serving kernel metrics endpoint");
    ...
    if let Err(error) = metrics_task.await {
        if !error.is_cancelled() {
            warn!(?error, "metrics server task terminated unexpectedly");
        }
    }
