...
| --- | --- | --- | --- |
| Control Plane (TypeScript/Node) | API, auth, product engineers | `apps/control-plane` | CRUD timers, validation, policy hooks, gRPC scheduling |
| Horology Kernel (Rust) | Systems, distributed systems | `services/horology-kernel` | Scheduler, gRPC surface, persistence adapters |
| Event Fabric & Orchestrators (TypeScript) | Integrations, reliability | `services/action-orchestrator` | gRPC/NATS listeners, action runners, retries |
...
- **Control Plane service** (`apps/control-plane`) with Express + Zod validation and a gRPC gateway into the Rust kernel.
- **Horology Kernel crate** (`services/horology-kernel`) implementing an async scheduler with tonic gRPC APIs and lifecycle events.
- **Action Orchestrator service** (`services/action-orchestrator`) that listens to kernel gRPC streams (with NATS/STDIN fallbacks) and performs HTTP/webhook actions.
...
3. **Run the stack locally**
   - Horology kernel: `cargo run --bin kernel` (listens on `0.0.0.0:50051` unless `KERNEL_GRPC_ADDR` is set).
   - Control plane: `npm run dev` inside `apps/control-plane` (listens on `localhost:4000` and targets the kernel gRPC server).
   - Action orchestrator: `npm run dev` inside `services/action-orchestrator` (consumes the kernel event stream or `NATS_URL`).
...
## 7. Next Steps
- Flesh out Postgres migrations and repository adapters in the control plane so timers are queryable beyond the kernel snapshot.
