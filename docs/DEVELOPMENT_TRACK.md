...
- **Horology Kernel crate** (`services/horology-kernel`) implementing an async scheduler with Tokio, broadcast timer events, and
  ...
  automatically restores/schedules timers after restarts. The kernel now accepts `ReportTimerExecution` gRPC calls so
  orchestrators can mark timers as failed and persist execution metadata.
- **Action Orchestrator service** (`services/action-orchestrator`) that listens to timer events, performs HTTP/webhook actions,
  and records execution traces. The `TimerActionExecutor` abstraction orchestrates webhook + agent command execution with
  structured logging and now reports aggregated action results/failures back to the kernel.
