...
### Kernel RPC authentication headers
The control plane now signs every RPC call to the kernel using derived headers:

- `x-tenant-id`
- `x-principal-id`
- `x-signature` (SHA256 hash of `principalId:tenantId`)

Local tooling that talks directly to the kernel must include the same header trio. The kernel rejects unsigned requests with `UNAUTHENTICATED`, preventing rogue clients from bypassing the policy wall.
