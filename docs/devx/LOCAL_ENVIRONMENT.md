...
- Import `infrastructure/grafana/dashboards/kernel-coordinator.json` into Grafana to visualise election attempts, leadership
  transitions, and heartbeat health for each node. The panels expect Prometheus naming conventions from the OTEL exporter.
- Use the sample collector profile in `infrastructure/otel/collector-honeycomb.yaml` when forwarding traces/metrics to
  Honeycomb or Grafana Cloud—set the environment variables noted in the file before launching the collector.
...
node -r ./scripts/ts-node-preload.js \
...
gRPC, and follower checks, along with quota enforcement output.

## 8. Kernel failover harness

Run `tests/phase1/runKernelFailoverHarness.ts` to spin up two real kernel binaries (Postgres-backed), elect a leader, trigger
failover, and assert that the follower assumes leadership and accepts timer schedules. The script:

- Builds the kernel binary with `cargo build --bin kernel` (debug profile).
- Starts two kernels with unique gRPC/metrics ports and shared Postgres advisory-lock coordination.
- Schedules timers against both nodes to confirm a single leader while validating follower `FAILED_PRECONDITION` responses.
- Sends `SIGINT` to the active leader and waits for the remaining node to win the lock before verifying new schedules succeed.
- Cleans up Postgres tables (`timer_records`, `timer_command_log`, `tenant_quota_usage`, `kernel_raft_state`) after the run.

Invoke it via the npm helper or the explicit Node command:

```bash
npm run test:phase1:failover
# or
NODE_PATH=$PWD/apps/control-plane/node_modules \
TS_NODE_PROJECT=apps/control-plane/tsconfig.json \
node -r ./scripts/ts-node-preload.js \
     -r ts-node/register/transpile-only \
     tests/phase1/runKernelFailoverHarness.ts
```

Ensure Postgres is running and migrations have been applied beforehand. The harness emits kernel logs with `[kernel-a]` /
`[kernel-b]` prefixes so you can observe the election timeline while the script waits for failover.
