...
### 🧠 Phase 5 Parserator Flywheel (2025-11-05)
- Parserator sources live under `/teams/:teamId/parserator/sources` with configurable mappings, scheduling policies, and webhook secrets (rotate without downtime).
- Public webhook endpoint (`POST /parserator/webhook?source=...`) validates HMAC signatures, persists structured events, and queues timer actions for automation.
- `processParseratorActions` Cloud Scheduler promotes pending actions into real timers while stamping Parserator metadata for downstream analytics—now backed by unit tests and a composite Firestore index on `status/scheduledFor`.
- New endpoints (`/parserator/events`, `/parserator/actions`, `/parserator/actions/:id/replay`) and SDK helpers surface a triage console for ops teams to review, replay, or cancel automation.
- Parserator source deletion cascades through historical events/actions, preventing orphaned documents without manual cleanup steps.
- Node + Python SDKs expose `listParseratorSources`, `previewParseratorSource`, `updateParseratorActionStatus`, and `replayParseratorAction` so orchestrations can be scripted end-to-end.
...
*Last updated: 2025-11-05 - Phase 5 parserator flywheel live alongside enterprise features*