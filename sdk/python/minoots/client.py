...
class AsyncMinootsClient:
    ...
    def _resolve_team(self, team: Optional[str], action: str) -> str:
        resolved = team or self._team
        if not resolved:
            raise MinootsError(f"team is required to {action}")
        return resolved

    async def list_parserator_sources(self, team: Optional[str] = None) -> list[Dict[str, Any]]:
        resolved = self._resolve_team(team, "list parserator sources")
        payload = await self._request("GET", f"/teams/{resolved}/parserator/sources")
        return payload.get("sources", [])

    async def create_parserator_source(
        self,
        team: Optional[str] = None,
        **config: Any,
    ) -> Dict[str, Any]:
        resolved = self._resolve_team(team, "create parserator source")
        payload = await self._request("POST", f"/teams/{resolved}/parserator/sources", json=config)
        return payload.get("source", {})

    async def get_parserator_source(self, source_id: str, *, team: Optional[str] = None) -> Dict[str, Any]:
        if not source_id:
            raise MinootsError("source_id is required")
        resolved = self._resolve_team(team, "fetch parserator source")
        payload = await self._request("GET", f"/teams/{resolved}/parserator/sources/{source_id}")
        return payload.get("source", {})

    async def update_parserator_source(
        self,
        source_id: str,
        *,
        team: Optional[str] = None,
        **updates: Any,
        ...
        if not source_id:
            raise MinootsError("source_id is required")
        resolved = self._resolve_team(team, "update parserator source")
        payload = await self._request("PATCH", f"/teams/{resolved}/parserator/sources/{source_id}", json=updates)
        return payload.get("source", {})

    async def rotate_parserator_source_secret(self, source_id: str, *, team: Optional[str] = None) -> Dict[str, Any]:
        if not source_id:
            raise MinootsError("source_id is required")
        resolved = self._resolve_team(team, "rotate parserator secret")
        payload = await self._request("POST", f"/teams/{resolved}/parserator/sources/{source_id}/rotate-secret")
        return payload.get("source", {})

    async def delete_parserator_source(self, source_id: str, *, team: Optional[str] = None) -> Dict[str, int]:
        if not source_id:
            raise MinootsError("source_id is required")
        resolved = self._resolve_team(team, "delete parserator source")
        payload = await self._request("DELETE", f"/teams/{resolved}/parserator/sources/{source_id}")
        return {
            "events_deleted": int(payload.get("eventsDeleted", 0)),
            "actions_deleted": int(payload.get("actionsDeleted", 0)),
        }

    async def preview_parserator_source(
        self,
        source_id: str,
        *,
        team: Optional[str] = None,
        ...
        if not source_id:
            raise MinootsError("source_id is required")
        resolved = self._resolve_team(team, "preview parserator source")
        payload = await self._request(
            "POST",
            f"/teams/{resolved}/parserator/sources/{source_id}/preview",
            json=sample_payload or {},
        )
        return payload.get("actions", [])

    async def list_parserator_events(
        self,
        *,
        team: Optional[str] = None,
        status: Optional[str] = None,
        ...
        resolved = self._resolve_team(team, "list parserator events")
        params: Dict[str, Any] = {}
        if status:
            params["status"] = status
        if source_id:
        ...
        if limit:
            params["limit"] = limit
        payload = await self._request("GET", f"/teams/{resolved}/parserator/events", params=params)
        return payload.get("events", [])

    async def list_parserator_actions(
        self,
        *,
        team: Optional[str] = None,
        status: Optional[str] = None,
        ...
        resolved = self._resolve_team(team, "list parserator actions")
        params: Dict[str, Any] = {}
        if status:
            params["status"] = status
        if source_id:
        ...
        if limit:
            params["limit"] = limit
        payload = await self._request("GET", f"/teams/{resolved}/parserator/actions", params=params)
        return payload.get("actions", [])

    async def update_parserator_action_status(
        self,
        action_id: str,
        *,
        team: Optional[str] = None,
        status: Optional[str] = None,
        defer_until: Optional[str | int | float | None] = None,
        result: Optional[Dict[str, Any]] = None,
        notes: Optional[str] = None,
        reset_attempts: bool = False,
    ) -> Dict[str, Any]:
        if not action_id:
            raise MinootsError("action_id is required")
        resolved = self._resolve_team(team, "update parserator action")
        payload: Dict[str, Any] = {}
        if status is not None:
            payload["status"] = status
        if defer_until is not None:
            payload["deferUntil"] = defer_until
        if result is not None:
            payload["result"] = result
        if notes is not None:
            payload["notes"] = notes
        if reset_attempts:
            payload["resetAttempts"] = True
        response = await self._request("POST", f"/teams/{resolved}/parserator/actions/{action_id}/status", json=payload)
        return response.get("action", {})

    async def replay_parserator_action(
        self,
        action_id: str,
        *,
        team: Optional[str] = None,
        ...
        if not action_id:
            raise MinootsError("action_id is required")
        resolved = self._resolve_team(team, "replay parserator action")
        payload: Dict[str, Any] = {}
        ...
        response = await self._request(
            "POST",
            f"/teams/{resolved}/parserator/actions/{action_id}/replay",
            json=payload,
        )
        return response.get("action", {})
