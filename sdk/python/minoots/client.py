...
class AsyncMinootsClient:
    ...
    async def replay_timer(
        ...
        **extra: Any,
        ...
        return await self._request("POST", f"/timers/{timer_id}/replay", json=payload)
    ...
    async def list_webhook_templates(self, team_id: str) -> Iterable[Dict[str, Any]]:
        if not team_id:
            raise MinootsError("team_id is required to list webhook templates")
        data = await self._request("GET", f"/teams/{team_id}/webhooks/templates")
        return data.get("templates", [])

    async def create_webhook(self, team_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to create webhooks")
        if not config.get("url"):
            raise MinootsError("Webhook url is required")
        return await self._request("POST", f"/teams/{team_id}/webhooks", json=config)

    async def create_webhook_from_template(
        self,
        team_id: str,
        template_key: str,
        ...
        if not team_id:
            raise MinootsError("team_id is required to create webhooks")
        if not template_key:
            raise MinootsError("template_key is required")
        if not overrides.get("url"):
            raise MinootsError("Webhook url is required for template creation")
        return await self._request(
            "POST",
            f"/teams/{team_id}/webhooks/templates/{template_key}",
            json=overrides,
        )

    async def list_webhooks(self, team_id: str) -> Iterable[Dict[str, Any]]:
        if not team_id:
            raise MinootsError("team_id is required to list webhooks")
        data = await self._request("GET", f"/teams/{team_id}/webhooks")
        return data.get("webhooks", [])

    async def update_webhook(self, team_id: str, webhook_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to update webhooks")
        if not webhook_id:
            raise MinootsError("webhook_id is required")
        data = await self._request(
            "PATCH",
            f"/teams/{team_id}/webhooks/{webhook_id}",
            json=updates,
        )
        return data.get("webhook", data)

    async def delete_webhook(self, team_id: str, webhook_id: str) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to delete webhooks")
        if not webhook_id:
            raise MinootsError("webhook_id is required")
        return await self._request("DELETE", f"/teams/{team_id}/webhooks/{webhook_id}")

    async def trigger_webhook_test(self, team_id: str, webhook_id: str) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to test webhooks")
        if not webhook_id:
            raise MinootsError("webhook_id is required")
        return await self._request("POST", f"/teams/{team_id}/webhooks/{webhook_id}/test")
    ...
    async def get_webhook_logs(
        ...
        *,
        ...
        if not team_id:
            raise MinootsError("team_id is required to fetch webhook logs")
        if not webhook_id:
            raise MinootsError("webhook_id is required")
        params: Dict[str, Any] = {}
        ...
        data = await self._request(
            "GET",
            f"/teams/{team_id}/webhooks/{webhook_id}/logs",
            params=params,
        )
        return data.get("logs", {})

    async def publish_event(self, team_id: str, event_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to publish events")
        if not event_type:
            raise MinootsError("event_type is required")
        return await self._request(
            "POST",
            f"/teams/{team_id}/events/{event_type}/publish",
            json=payload,
        )

    async def list_integrations(self, team_id: str) -> Iterable[Dict[str, Any]]:
        if not team_id:
            raise MinootsError("team_id is required to list integrations")
        data = await self._request("GET", f"/teams/{team_id}/integrations")
        return data.get("integrations", [])

    async def upsert_integration(self, team_id: str, integration_type: str, config: Dict[str, Any]) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to configure integrations")
        if not integration_type:
            raise MinootsError("integration type is required")
        data = await self._request(
            "PUT",
            f"/teams/{team_id}/integrations/{integration_type}",
            json=config,
        )
        return data.get("integration", data)

    async def delete_integration(self, team_id: str, integration_type: str) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to delete integrations")
        if not integration_type:
            raise MinootsError("integration type is required")
        return await self._request("DELETE", f"/teams/{team_id}/integrations/{integration_type}")

    async def test_integration(self, team_id: str, integration_type: str) -> Dict[str, Any]:
        if not team_id:
            raise MinootsError("team_id is required to test integrations")
        if not integration_type:
            raise MinootsError("integration type is required")
        return await self._request(
            "POST",
            f"/teams/{team_id}/integrations/{integration_type}/test",
        )
    ...
    async def notify_integration(
        ...
        if not team_id:
            raise MinootsError("team_id is required to notify integrations")
        if not integration_type:
            raise MinootsError("integration type is required")
        return await self._request(
            "POST",
            f"/teams/{team_id}/integrations/{integration_type}/notify",
            json=message,
        )
