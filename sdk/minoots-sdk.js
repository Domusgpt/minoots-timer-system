...
class MinootsSDK {
    ...
    _requireTeam(teamId, action) {
        const resolved = teamId || this.defaultTeam;
        if (!resolved) {
            throw new MinootsError(`teamId is required to ${action}`);
        }
        return resolved;
    }

    async listParseratorSources(teamId) {
        const resolvedTeam = this._requireTeam(teamId, 'list parserator sources');
        const payload = await this._request('GET', `/teams/${resolvedTeam}/parserator/sources`);
        return payload.sources || [];
    }

    async createParseratorSource(teamId, config = {}) {
        const resolvedTeam = this._requireTeam(teamId, 'create parserator source');
        const payload = await this._request('POST', `/teams/${resolvedTeam}/parserator/sources`, { body: config });
        return payload.source;
    }

    async getParseratorSource(teamId, sourceId) {
        if (!sourceId) {
            throw new MinootsError('sourceId is required to fetch parserator source');
        }
        const resolvedTeam = this._requireTeam(teamId, 'fetch parserator source');
        const payload = await this._request('GET', `/teams/${resolvedTeam}/parserator/sources/${sourceId}`);
        return payload.source;
    }

    async updateParseratorSource(teamId, sourceId, updates = {}) {
        if (!sourceId) {
            throw new MinootsError('sourceId is required to update parserator source');
        }
        const resolvedTeam = this._requireTeam(teamId, 'update parserator source');
        const payload = await this._request('PATCH', `/teams/${resolvedTeam}/parserator/sources/${sourceId}`, { body: updates });
        return payload.source;
    }

    async rotateParseratorSourceSecret(teamId, sourceId) {
        if (!sourceId) {
            throw new MinootsError('sourceId is required to rotate parserator secret');
        }
        const resolvedTeam = this._requireTeam(teamId, 'rotate parserator secret');
        const payload = await this._request('POST', `/teams/${resolvedTeam}/parserator/sources/${sourceId}/rotate-secret`);
        return payload.source;
    }

    async deleteParseratorSource(teamId, sourceId) {
        if (!sourceId) {
            throw new MinootsError('sourceId is required to delete parserator source');
        }
        const resolvedTeam = this._requireTeam(teamId, 'delete parserator source');
        const payload = await this._request('DELETE', `/teams/${resolvedTeam}/parserator/sources/${sourceId}`);
        return {
            actionsDeleted: payload.actionsDeleted || 0,
            eventsDeleted: payload.eventsDeleted || 0,
        };
    }

    async previewParseratorSource(teamId, sourceId, samplePayload = {}) {
        if (!sourceId) {
            throw new MinootsError('sourceId is required to preview parserator mapping');
        }
        const resolvedTeam = this._requireTeam(teamId, 'preview parserator mapping');
        const payload = await this._request('POST', `/teams/${resolvedTeam}/parserator/sources/${sourceId}/preview`, { body: samplePayload });
        return payload.actions || [];
    }

    async listParseratorEvents(teamId, options = {}) {
        const resolvedTeam = this._requireTeam(teamId, 'list parserator events');
        const query = {};
        if (options.status) query.status = options.status;
        if (options.sourceId) query.sourceId = options.sourceId;
        if (options.limit) query.limit = options.limit;
        const payload = await this._request('GET', `/teams/${resolvedTeam}/parserator/events`, { query });
        return payload.events || [];
    }

    async listParseratorActions(teamId, options = {}) {
        const resolvedTeam = this._requireTeam(teamId, 'list parserator actions');
        const query = {};
        if (options.status) query.status = options.status;
        if (options.sourceId) query.sourceId = options.sourceId;
        if (options.limit) query.limit = options.limit;
        const payload = await this._request('GET', `/teams/${resolvedTeam}/parserator/actions`, { query });
        return payload.actions || [];
    }

    async updateParseratorActionStatus(teamId, actionId, updates = {}) {
        if (!actionId) {
            throw new MinootsError('actionId is required to update parserator action');
        }
        const resolvedTeam = this._requireTeam(teamId, 'update parserator action');
        const payload = await this._request('POST', `/teams/${resolvedTeam}/parserator/actions/${actionId}/status`, { body: updates });
        return payload.action;
    }

    async replayParseratorAction(teamId, actionId, options = {}) {
        if (!actionId) {
            throw new MinootsError('actionId is required to replay parserator action');
        }
        const resolvedTeam = this._requireTeam(teamId, 'replay parserator action');
        const payload = await this._request('POST', `/teams/${resolvedTeam}/parserator/actions/${actionId}/replay`, { body: options });
        return payload.action;
    }
