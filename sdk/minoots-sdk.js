...
class MinootsSDK {
    ...
    async replayTimer(timerId, options = {}) {
        if (!timerId) {
            throw new MinootsError('Timer ID is required');
        }
        const payload = {};
        ...
        if (Array.isArray(options.dependencies)) payload.dependencies = options.dependencies;
        if (options.retryPolicy) payload.retryPolicy = options.retryPolicy;
        if (options.team) payload.team = options.team;
        if (options.agentId) payload.agentId = options.agentId;
        return await this._request('POST', `/timers/${timerId}/replay`, { body: payload });
    }
    ...
    async listWebhookTemplates(teamId) {
        if (!teamId) {
            throw new MinootsError('teamId is required to list webhook templates');
        }
        const payload = await this._request('GET', `/teams/${teamId}/webhooks/templates`);
        return payload.templates || [];
    }

    async createWebhook(teamId, config = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to create webhooks');
        }
        if (!config.url) {
            throw new MinootsError('Webhook url is required');
        }
        return await this._request('POST', `/teams/${teamId}/webhooks`, { body: config });
    }

    async createWebhookFromTemplate(teamId, templateKey, overrides = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to create webhooks');
        }
        if (!templateKey) {
            throw new MinootsError('templateKey is required');
        }
        if (!overrides.url) {
            throw new MinootsError('Webhook url is required');
        }
        return await this._request('POST', `/teams/${teamId}/webhooks/templates/${templateKey}`, { body: overrides });
    }

    async listWebhooks(teamId) {
        if (!teamId) {
            throw new MinootsError('teamId is required to list webhooks');
        }
        const payload = await this._request('GET', `/teams/${teamId}/webhooks`);
        return payload.webhooks || [];
    }

    async updateWebhook(teamId, webhookId, updates = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to update webhooks');
        }
        if (!webhookId) {
            throw new MinootsError('webhookId is required');
        }
        const payload = await this._request('PATCH', `/teams/${teamId}/webhooks/${webhookId}`, { body: updates });
        return payload.webhook;
    }

    async deleteWebhook(teamId, webhookId) {
        if (!teamId) {
            throw new MinootsError('teamId is required to delete webhooks');
        }
        if (!webhookId) {
            throw new MinootsError('webhookId is required');
        }
        return await this._request('DELETE', `/teams/${teamId}/webhooks/${webhookId}`);
    }

    async triggerWebhookTest(teamId, webhookId) {
        if (!teamId) {
            throw new MinootsError('teamId is required to test webhooks');
        }
        if (!webhookId) {
            throw new MinootsError('webhookId is required');
        }
        return await this._request('POST', `/teams/${teamId}/webhooks/${webhookId}/test`);
    }

    async getWebhookLogs(teamId, webhookId, options = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to fetch webhook logs');
        }
        if (!webhookId) {
            throw new MinootsError('webhookId is required');
        }
        ...
        const payload = await this._request('GET', `/teams/${teamId}/webhooks/${webhookId}/logs`, { query });
        return payload.logs;
    }

    async publishEvent(teamId, eventType, payload = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to publish events');
        }
        if (!eventType) {
            throw new MinootsError('eventType is required');
        }
        return await this._request('POST', `/teams/${teamId}/events/${eventType}/publish`, { body: payload });
    }

    async listIntegrations(teamId) {
        if (!teamId) {
            throw new MinootsError('teamId is required to list integrations');
        }
        const payload = await this._request('GET', `/teams/${teamId}/integrations`);
        return payload.integrations || [];
    }

    async upsertIntegration(teamId, type, config = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to configure integrations');
        }
        if (!type) {
            throw new MinootsError('integration type is required');
        }
        const payload = await this._request('PUT', `/teams/${teamId}/integrations/${type}`, { body: config });
        return payload.integration;
    }

    async deleteIntegration(teamId, type) {
        if (!teamId) {
            throw new MinootsError('teamId is required to delete integrations');
        }
        if (!type) {
            throw new MinootsError('integration type is required');
        }
        return await this._request('DELETE', `/teams/${teamId}/integrations/${type}`);
    }

    async testIntegration(teamId, type) {
        if (!teamId) {
            throw new MinootsError('teamId is required to test integrations');
        }
        if (!type) {
            throw new MinootsError('integration type is required');
        }
        return await this._request('POST', `/teams/${teamId}/integrations/${type}/test`);
    }

    async notifyIntegration(teamId, type, message = {}) {
        if (!teamId) {
            throw new MinootsError('teamId is required to notify integrations');
        }
        if (!type) {
            throw new MinootsError('integration type is required');
        }
        return await this._request('POST', `/teams/${teamId}/integrations/${type}/notify`, { body: message });
    }
