...
The Node SDK now ships a `minoots` CLI for CI/CD pipelines and local debugging.
...
```bash
# Create a timer and persist its ID to ./.minoots/timer-id
minoots timers create --team team_123 --name build --duration 10m --metadata branch=main

# Wait for a timer to finish using the persisted ID
minoots timers wait --team team_123 --timer $(cat .minoots/timer-id)

# Manage team webhooks
minoots webhooks list --team team_123
minoots webhooks create --team team_123 --name deploy --url https://hooks.example.com

# Configure integrations
minoots integrations set --team team_123 --provider slack --config webhookUrl=https://hooks.slack.com/services/... 
```

Set `MINOOTS_API_KEY` (and optionally `MINOOTS_API_BASE`) before invoking the CLI. The `MINOOTS_CLI_FETCH` environment variable can point to a CommonJS module exporting a custom `fetch` implementation when writing automated tests.
