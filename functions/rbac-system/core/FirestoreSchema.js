...
class FirestoreSchemaManager {
  ...
  /**
   * Invite user to organization by email (creates invitation record)
   */
  async inviteUserToOrganization(organizationId, email, role = 'editor', invitedBy) {
    try {
      ...
      try {
        ...
        console.log(`User with email ${email} not found in Firebase Auth, will create invitation`);
      }
      ...
      // If user exists, add them directly and mark invitation as accepted
      if (userId) {
        await this.addUserToOrganization(userId, organizationId, role);
        invitation.status = 'accepted';
        invitation.acceptedAt = admin.firestore.FieldValue.serverTimestamp();
        invitation.userId = userId;
      }

      // Store invitation
      await this.db.collection('invitations').doc(invitationId).set(invitation);
      ...
      return {
        ...
        message: userId ? 
          ...
          'Invitation created. User will be added when they sign up.'
      };
