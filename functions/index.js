...
const { registerWebhook, listWebhooks, getWebhook, updateWebhook, deleteWebhook, publishEvent, dispatchQueuedDeliveries, triggerTestDelivery, getWebhookLogs, listWebhookTemplates, createWebhookFromTemplate } = require('./utils/webhooks');
const { saveIntegration, listIntegrations, deleteIntegration, sendIntegrationNotification, sendIntegrationTest } = require('./utils/integrations');
...
// Webhook management (Phase 6 integrations)
app.get('/teams/:teamId/webhooks/templates', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), (req, res) => {
    res.json({ success: true, templates: listWebhookTemplates() });
});

app.post('/teams/:teamId/webhooks/templates/:templateKey', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, templateKey } = req.params;
    try {
        const result = await createWebhookFromTemplate(teamId, templateKey, req.body || {});
        res.status(201).json({ success: true, ...result });
    } catch (error) {
        console.error('Create webhook from template error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.get('/teams/:teamId/webhooks', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    try {
...
app.post('/teams/:teamId/webhooks', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    try {
        const result = await registerWebhook(teamId, req.body || {});
        res.status(201).json({ success: true, ...result });
    } catch (error) {
...
app.patch('/teams/:teamId/webhooks/:webhookId', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, webhookId } = req.params;
    try {
        const result = await updateWebhook(teamId, webhookId, req.body || {});
...
app.delete('/teams/:teamId/webhooks/:webhookId', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, webhookId } = req.params;
    try {
        const deleted = await deleteWebhook(teamId, webhookId);
...
app.get('/teams/:teamId/webhooks/:webhookId/logs', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, webhookId } = req.params;
    const { limit, cursor } = req.query;
    try {
        const logs = await getWebhookLogs(teamId, webhookId, {
...
app.post('/teams/:teamId/webhooks/:webhookId/test', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, webhookId } = req.params;
    try {
        const result = await triggerTestDelivery(teamId, webhookId);
        res.json({ success: true, ...result });
    } catch (error) {
        console.error('Trigger webhook test error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.post('/teams/:teamId/events/:eventType/publish', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, eventType } = req.params;
    try {
        const result = await publishEvent(teamId, eventType, req.body || {});
        res.status(202).json({ success: true, ...result });
    } catch (error) {
        console.error('Publish webhook event error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

// Integration configuration endpoints
app.get('/teams/:teamId/integrations', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    try {
        const integrations = await listIntegrations(teamId);
...
app.put('/teams/:teamId/integrations/:type', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, type } = req.params;
    try {
        const integration = await saveIntegration(teamId, type, req.body || {});
...
app.delete('/teams/:teamId/integrations/:type', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, type } = req.params;
    try {
        const deleted = await deleteIntegration(teamId, type);
        res.json({ success: true, deleted });
    } catch (error) {
...
app.post('/teams/:teamId/integrations/:type/test', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, type } = req.params;
    try {
        const response = await sendIntegrationTest(teamId, type);
...
app.post('/teams/:teamId/integrations/:type/notify', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, type } = req.params;
    try {
        const response = await sendIntegrationNotification(teamId, type, req.body || {});
        res.json({ success: true, response });
    } catch (error) {
...
exports.processWebhookQueue = onSchedule('every 1 minutes', async () => {
        const result = await dispatchQueuedDeliveries({ limit: 50 });
        if (result.processed > 0) {
            console.log('Processed webhook queue', result);
        }
        return null;
    });
