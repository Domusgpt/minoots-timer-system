...
// Parserator integration endpoints (Phase 5)
app.get('/teams/:teamId/parserator/sources', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    try {
        const sources = await listParseratorSources(teamId);
        res.json({ success: true, sources });
    } catch (error) {
        console.error('List parserator sources error:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});

app.post('/teams/:teamId/parserator/sources', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    try {
        const source = await createParseratorSource(teamId, req.body || {}, req.user.id);
        res.status(201).json({ success: true, source });
    } catch (error) {
        console.error('Create parserator source error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.get('/teams/:teamId/parserator/sources/:sourceId', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), async (req, res) => {
    const { teamId, sourceId } = req.params;
    try {
        const source = await getParseratorSource(teamId, sourceId);
        res.json({ success: true, source });
    } catch (error) {
        console.error('Get parserator source error:', error);
        res.status(404).json({ success: false, error: error.message });
    }
});

app.patch('/teams/:teamId/parserator/sources/:sourceId', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, sourceId } = req.params;
    try {
        const source = await updateParseratorSource(teamId, sourceId, req.body || {}, req.user.id);
        res.json({ success: true, source });
    } catch (error) {
        console.error('Update parserator source error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.post('/teams/:teamId/parserator/sources/:sourceId/rotate-secret', requireTeamRole((req) => req.params.teamId, ['owner']), async (req, res) => {
    const { teamId, sourceId } = req.params;
    try {
        const source = await rotateParseratorSourceSecret(teamId, sourceId, req.user.id);
        res.json({ success: true, source });
    } catch (error) {
        console.error('Rotate parserator secret error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.post('/teams/:teamId/parserator/sources/:sourceId/preview', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, sourceId } = req.params;
    try {
        const preview = await previewParseratorMapping(teamId, sourceId, req.body || {});
        res.json({ success: true, actions: preview });
    } catch (error) {
        console.error('Preview parserator mapping error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.delete('/teams/:teamId/parserator/sources/:sourceId', requireTeamRole((req) => req.params.teamId, ['owner']), async (req, res) => {
    const { teamId, sourceId } = req.params;
    try {
        const result = await deleteParseratorSource(teamId, sourceId);
        res.json({ success: true, ...result });
    } catch (error) {
        console.error('Delete parserator source error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.get('/teams/:teamId/parserator/events', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    const { status, sourceId, limit } = req.query || {};
    try {
        const parsedLimit = limit ? Math.min(parseInt(limit, 10) || 25, 100) : undefined;
        const events = await listParseratorEvents(teamId, { status, sourceId, limit: parsedLimit });
        res.json({ success: true, events });
    } catch (error) {
        console.error('List parserator events error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.get('/teams/:teamId/parserator/actions', requireTeamRole((req) => req.params.teamId, ['member', 'admin', 'owner']), async (req, res) => {
    const { teamId } = req.params;
    const { status, sourceId, limit } = req.query || {};
    try {
        const parsedLimit = limit ? Math.min(parseInt(limit, 10) || 25, 100) : undefined;
        const actions = await listParseratorActions(teamId, { status, sourceId, limit: parsedLimit });
        res.json({ success: true, actions });
    } catch (error) {
        console.error('List parserator actions error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.post('/teams/:teamId/parserator/actions/:actionId/status', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, actionId } = req.params;
    try {
        const action = await updateParseratorActionStatus(teamId, actionId, req.body || {}, req.user.id);
        res.json({ success: true, action });
    } catch (error) {
        console.error('Update parserator action status error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});

app.post('/teams/:teamId/parserator/actions/:actionId/replay', requireTeamRole((req) => req.params.teamId, ['admin', 'owner']), async (req, res) => {
    const { teamId, actionId } = req.params;
    try {
        const action = await replayParseratorAction(teamId, actionId, req.body || {}, req.user.id);
        res.json({ success: true, action });
    } catch (error) {
        console.error('Replay parserator action error:', error);
        res.status(400).json({ success: false, error: error.message });
    }
});
...
app.post('/parserator/webhook', async (req, res) => {
    const sourceId = req.query.source || req.headers['x-parserator-source'];
    const signature = req.headers['x-parserator-signature'];
    try {
        const result = await handleParseratorWebhook({
            sourceId,
            rawBody: req.rawBody,
            payload: req.body,
            signature,
        ...
        console.error('Parserator webhook error:', error);
...
async function processParseratorActionsHandler({ dbInstance = db, timer = RealTimer } = {}) {
    const nowTs = admin.firestore.Timestamp.now();
    const snapshot = await dbInstance.collectionGroup('parseratorActions')
    ...
    console.log(`Processing ${snapshot.size} parserator actions`);
    let processed = 0;
    ...
    for (const doc of snapshot.docs) {
        ...
        await dbInstance.runTransaction(async (tx) => {
            ...
            actionData = { id: fresh.id, ...current };
            tx.update(doc.ref, {
                status: 'processing',
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        ...
        try {
            const timerMetadata = {
                ...(actionData.timer?.metadata || {}),
                parseratorActionId: actionData.id,
                parseratorSourceId: actionData.sourceId,
                parseratorEventId: actionData.eventId,
            };
            const timerPayload = {
                ...(actionData.timer || {}),
                team: actionData.teamId || actionData.timer?.team || null,
                metadata: timerMetadata,
            };
            // eslint-disable-next-line no-await-in-loop
            const timerResult = await timer.create({
                ...timerPayload,
                createdBy: actionData.createdBy || 'parserator_bridge',
            });
            // eslint-disable-next-line no-await-in-loop
            await doc.ref.set({
                status: 'completed',
                result: { timerId: timerResult.id },
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            }, { merge: true });
            processed += 1;
        } catch (error) {
            console.error('Failed to execute parserator action', actionData.id, error);
            // eslint-disable-next-line no-await-in-loop
            await doc.ref.set({
                status: 'failed',
...
exports.processParseratorActions = onSchedule('every 1 minutes', async () => processParseratorActionsHandler());
...
exports.__test = {
    processParseratorActionsHandler,
};
