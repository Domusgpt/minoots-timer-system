...
### Entry #8: Kernel failover harness + observability packs - COMPLETED
**Time:** 2025-10-18 12:05-14:10 UTC
**Task:** Execute the Wave 1 follow-ups by validating multi-node failover end-to-end and publishing ready-to-import observability assets.
**Status:** ✅ COMPLETED

**Actions:**
1. Implemented `tests/phase1/runKernelFailoverHarness.ts` to build two kernel binaries, verify single-leader semantics, trigger a SIGINT failover, and assert the follower accepts new schedules while cleaning Postgres state.
2. Added `npm run test:phase1:failover`, refreshed the local environment guide with execution steps, and documented the harness log prefixes for debugging.
3. Published a Prometheus-backed Grafana dashboard (`infrastructure/grafana/dashboards/kernel-coordinator.json`) and an OTEL collector profile for Honeycomb/Grafana Cloud forwarding (`infrastructure/otel/collector-honeycomb.yaml`).

**Tests Performed:**
- ✅ `cargo test --manifest-path services/horology-kernel/Cargo.toml -- --test-threads=1`
- ✅ `npm test -- --passWithNoTests`
- ✅ `npm run build`
- ✅ `node scripts/check-devlog-entry.js`
...
**Next Steps:**
- Integrate the failover harness into CI alongside the existing Phase 1 run once multi-container runners are configured.
- Extend the harness to scrape the metrics endpoint post-failover and assert election counters increment as expected.
- Backfill Raft supervisor wiring into the failover run when the OpenRaft integration replaces the Postgres coordinator.
