...
class HttpMinootsClient implements MinootsClient {
  ...
  constructor(options: HttpClientOptions) {
    ...
    this.useCredentials = options.useCredentials ?? !this.apiKey;
  }
  ...
  private buildHeaders(headers?: HeadersInit): HeadersInit {
    ...
    if (headers instanceof Headers) {
      const combined = new Headers(defaultHeaders);
      headers.forEach((value, key) => combined.set(key, value));
      return combined;
    }
    if (Array.isArray(headers)) {
      const combined = new Headers(defaultHeaders);
      headers.forEach(([key, value]) => combined.set(key, value));
      return combined;
    }
  ...
  onTimerEvents(handler: (event: TimerEvent) => void): () => void {
    const url = new URL('/timers/stream', `${this.baseUrl}/`);
    if (this.apiKey) {
      url.searchParams.set('apiKey', this.apiKey);
    }
    const eventSourceInit: EventSourceInit | undefined = this.useCredentials ? { withCredentials: true } : undefined;
    const eventSource = new EventSource(url.toString(), eventSourceInit);
...
export interface MinootsClientOptions {
  baseUrl?: string;
  apiKey?: string;
  useCredentials?: boolean;
  forceMock?: boolean;
...
export function createMinootsClient(options?: MinootsClientOptions): MinootsClient {
  if (options?.forceMock) {
    return new MockMinootsClient();
  }

  const baseUrl = options?.baseUrl ?? (import.meta.env.VITE_MINOOTS_API_BASE as string | undefined);
  if (baseUrl) {
    const apiKey = options?.apiKey ?? (import.meta.env.VITE_MINOOTS_API_KEY as string | undefined);
    const useCredentialsEnv = import.meta.env.VITE_MINOOTS_USE_CREDENTIALS as string | undefined;
    const useCredentials = options?.useCredentials ?? (useCredentialsEnv ? useCredentialsEnv.toLowerCase() === 'true' : undefined);
    return new HttpMinootsClient({ baseUrl, apiKey, useCredentials });
  }
