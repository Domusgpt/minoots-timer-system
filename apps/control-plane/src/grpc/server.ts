    ...
    StreamTimerEvents: (call: grpc.ServerWritableStream<any, any>) => {
      (async () => {
        ...
        try {
          ...
          const eventStream = kernelClient.streamTimerEvents(
            { tenantId: context.tenantId, topics: call.request?.topics ?? [] },
            metadata,
          );
          stream = eventStream;
          eventStream.on('data', (message: any) => {
            call.write(message);
          });
...
const normalizeScheduleRequest = (request: any, tenantId: string) => {
  const rawDuration =
    request.durationMs ?? request.duration_ms ?? request.scheduleTime?.durationMs ?? request.scheduleTime?.duration_ms;
  const parsedDuration =
    typeof rawDuration === 'string'
      ...
      : typeof rawDuration === 'number'
        ? rawDuration
        : undefined;
  const duration = typeof parsedDuration === 'number' && Number.isFinite(parsedDuration) ? parsedDuration : undefined;
  const payload = {
    tenantId,
...
const mapGrpcError = (error: any): grpc.ServiceError => {
  ...
  if (error instanceof KernelNotLeaderError) {
    const serviceError = new Error(error.message) as grpc.ServiceError;
    serviceError.code = grpc.status.FAILED_PRECONDITION;
    if (typeof error.retryAfterMs === 'number') {
      serviceError.metadata = new Metadata();
      serviceError.metadata.set('retry-after-ms', error.retryAfterMs.toString());
    }
    return serviceError;
  }
  ...
  logger.error({ error: serializeError(error) }, 'Unhandled gRPC gateway error');
