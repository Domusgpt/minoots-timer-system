...
export class FirebaseAuthMiddleware {
  ...
  private readonly anonymousUser: AuthenticatedUser;
  ...
  constructor(options: FirebaseAuthMiddlewareOptions = {}) {
    ...
    this.firebaseEnabled = options.firebaseEnabled ?? true;
    this.anonymousUser = options.anonymousUser ?? { id: 'anonymous', tier: 'anonymous' };
  }
      ...
      if (bearer) {
        if (!this.firebaseEnabled) {
          if (this.allowAnonymous) {
            req.auth = { user: this.anonymousUser, method: 'anonymous' };
            return next();
          }
          res.status(503).json({ message: 'Firebase authentication is disabled in this environment' });
          return;
        }
