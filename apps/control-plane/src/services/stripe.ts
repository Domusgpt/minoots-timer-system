...
interface StripeServiceOptions {
  enabled?: boolean;
}
...
export class StripeService {
  ...
  constructor(options: StripeServiceOptions = {}) {
    const desired = options.enabled ?? true;
    const client = desired ? stripeClient() : null;
    this.stripe = client;
...
export const createStripeService = (options?: StripeServiceOptions) => new StripeService(options);
